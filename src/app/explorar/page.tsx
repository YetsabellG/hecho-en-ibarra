...
export default async function ExplorePage({ searchParams }: { searchParams?: Promise<{ categoria?: string }> }) {
  ...
  return <main className="min-h-screen bg-[#f8f1e7]"><Header/><PageHero eyebrow="Explora Ibarra" title="Encuentra lo mejor de nuestra ciudad" description="Descubre tiendas, productos y servicios creados por personas de Ibarra."/><section className="mx-auto max-w-7xl px-5 py-14 lg:px-8"><div className="mt-10 flex gap-3 overflow-x-auto pb-2">{categories.map(({ name, icon: Icon }) => <Link key={name} href={`/explorar?categoria=${encodeURIComponent(name)}`} className={`flex shrink-0 items-center gap-2 rounded-full border px-4 py-2.5 text-sm font-bold ${category === name ? "border-[#891C20] bg-[#891C20] text-white" : "border-[#eadbca] bg-[#fffdf9] text-[#75685f] hover:border-[#891C20] hover:text-[#891C20]"}`}><Icon size={16}/>{name}</Link>)}</div>{category && <p className="mt-6 text-sm font-bold text-[#6f1519]">Categoría seleccionada: {category} · <Link href="/explorar" className="underline">Ver todas</Link></p>}{businessRows.length === 0 ? <div className="mt-12 rounded-[28px] border border-dashed border-[#d9c3ad] bg-[#fffdf9] p-16 text-center"><Sparkles className="mx-auto text-[#891C20]" size={38}/><h2 className="mt-4 text-2xl font-black">Próximamente habrá más emprendimientos</h2><p className="mx-auto mt-3 max-w-lg text-[#75685f]">Estamos preparando nuevos negocios locales para que encuentres más opciones en esta categoría.</p></div> : <div className="mt-12 grid gap-7 md:grid-cols-2 lg:grid-cols-3">{businessRows.map((business) => <BusinessCard key={business.id} name={business.name} category={business.category || "Emprendimiento local"} description={business.description || "Productos y servicios hechos en Ibarra."} image={business.image || "/ibarra-hero.png"} location={business.city || "Ibarra"} verified={business.verified || false} premium={business.premium || false} slug={business.slug} rating={business.rating} favorites={business.favorites} subcategories={productMap.get(business.id)?.map((product) => product.category).filter(Boolean) as string[] || []}/>)}</div>}</section></main>;
}
